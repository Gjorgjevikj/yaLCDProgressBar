#ifndef LCDPROGRESSTHINBAR_H
#define LCDPROGRESSTHINBAR_H

#include <Arduino.h>
#include <LiquidCrystal_I2C.h>     // if you don't have I2C version of the display, use LiquidCrystal.h library instead

class LCDProgressThinBar
{
public:
    enum PixelMatrix : byte { charRows = 8, charCols = 5 };
    enum CustomChar : byte { pbBlank, pbLeft, pbMiddle, pbRight, pbLeft2, pbMiddle2, pbRight2, pbFull };

    LCDProgressThinBar(LiquidCrystal_I2C& _display, byte _width, byte _row = 0, byte _col = 0, bool _alt = false);
    virtual void init();
    int size() const;
    void setWidth(int w);
...
protected:
    ...
    virtual void CreateCustomCharacter(uint8_t chpos, uint8_t mask, uint8_t vframe, uint8_t hframe);
